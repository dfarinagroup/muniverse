...
from .decomposition import decompose_cbss, decompose_scd, decompose_upperbound
    ...
    if method == "scd":
        ...
        if method == "upperbound":
            return decompose_upperbound(
                data=data, algorithm_config=algorithm_config, output_dir=output_dir
            )
        else:  # cbss
        ...
        raise ValueError(
            f"Unknown method: {method}. Must be one of: scd, upperbound, cbss"
        )
