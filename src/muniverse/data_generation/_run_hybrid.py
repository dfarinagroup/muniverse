...
def create_trapezoid_effort(
    ...
    if len(muscle_force) > expected_samples:
        ...
        muscle_force = np.pad(
            muscle_force, (0, expected_samples - len(muscle_force)), "constant"
        )
...
def exponential_sample_motor_units_by_index(muaps, num_to_select, seed, exp_factor=5.0):
    ...
    if num_to_select >= num_mus:
        print(
            f"Warning: Requested {num_to_select} MUs but only {num_mus} available. Using all available MUs."
        )
    ...
    print(
        f"Selected {num_to_select} motor units with index range: [{selected_indices.min()}-{selected_indices.max()}] of {num_mus}"
    )
    print(
        f"Index percentiles: 25%={np.percentile(index_percentages, 25):.1f}%, "
...
def generate_emg_signal(
    muaps, spikes, time_samples, noise_level_db=None, noise_seed=None
):
    """
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate EMG signals from curated MUAPs"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--muaps_file", type=str, required=True, help="Path to MUAPs file (npz format)"
    )
    parser.add_argument(
        "--pytorch-device",
        type=str,
    ...
    if ms_label != "Tibialis Anterior":
        print(
            f"Warning: This script is optimized for Tibialis Anterior, but '{ms_label}' was specified."
        )
    ...
    print(
        f"Selecting {num_mus} motor units with exponential sampling (factor={args.exp_factor})..."
    )
    ...
    if hasattr(electrode_cfg, "GridShape"):
        ...
        total_electrodes = getattr(
            electrode_cfg, "EMGChannelCount", selected_muaps.shape[1]
        )
    ...
    print(
        f"Initializing MotoneuronPool with {num_mus} motor units (dummy muscle: ECRB)..."
    )
    ...
    emg = generate_emg_signal(
        selected_muaps, spikes, len(effort_profile), noise_level_db, noise_seed
    )
    ...
    # Save all outputs
    save_outputs(args.output_dir, emg, spikes, effort_profile, cfg, metadata)
